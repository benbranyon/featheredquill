...
class MarketplacesPage extends AbstractAdminPage implements ComponentContract
{
    ...
    {
        $storeId = Marketplaces::getStoreId();
        $buttonBehavior = 'onclick="window.open(\''.esc_url(Marketplaces::getCommerceHubUrl("/home/sales-channels?storeId={$storeId}&ua_placement=shared_header")).'\', \'_blank\');"';
        ...
        if (! Marketplaces::areSettingsSupported()) {
            $buttonBehavior = 'disabled="disabled"';
            $refreshPageBtn = '';
        }

        $addSalesChannelBtn = '<button '.$buttonBehavior.' class="button button-primary">'.esc_html__('Add Sales Channel', 'mwc-core').'</button>';

        echo '<p class="manage-sales-channels-buttons">'.$addSalesChannelBtn.$refreshPageBtn.'</p>';
    }
