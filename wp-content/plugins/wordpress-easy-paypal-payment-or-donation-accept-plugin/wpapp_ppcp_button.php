...
function wpapp_load_ppcp_button( $args = array() ) {
    ...
    $txn_success_extra_msg = __('Feel free to browser our site for another checkout.', 'wordpress-accept-paypal-payment');
    ...
    $transient_timeout = 60 * 60 * 3; //3 hours
...
function wpapp_set_visitor_id_to_cookie(){
    if ( !isset($_COOKIE['wpapp_visitor_id']) || empty($_COOKIE['wpapp_visitor_id']) ) {
        ...
        setcookie('wpapp_visitor_id', $wpapp_global_visitor_id, time() + (86400 * 30), "/"); // 86400 = 1 day
        
    }
}

add_action('init', 'wpapp_set_visitor_id_to_cookie');//Set the visitor ID (if not already set).
