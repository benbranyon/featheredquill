...
function awards_post_thumb( $width=300, $height='', $html=true, $crop = true) {		
  global $post;
  if( !has_post_thumbnail() ) return false;

  $fullsize = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
  $url = awards_image_resize( $fullsize[0], $width, $height, $crop, 'c', false );
...
add_action('delete_attachment', 'awards_delete_resized_images');

function awards_image_resize($url, $width=null, $height=null, $crop=true, $align='c', $retina=false) {

  global $wpdb;

  // Get common vars (func_get_args() only get specified values)
  $common = awards_common_info($url, $width, $height, $crop, $align, $retina);
...
function awards_common_info($url, $width, $height, $crop, $align, $retina) {
...
function awards_delete_resized_images($post_id) {
