...
function awards_login_logo() { 
	...
	if($logo != ''){
	wp_enqueue_style('awards-custom-admin-style', AWARDSTHEMEURI . 'admin/assets/css/custom_admin_style.css');
	...
	wp_add_inline_style( 'awards-custom-admin-style', $custom_css );
	}
}
add_action( 'login_enqueue_scripts', 'awards_login_logo' );

function awards_options_filter($var){
...
function awards_custom_css_from_theme_options(){
	$custom_css = '';
	if( function_exists( 'ot_get_option' ) ):
    $settings = awards_theme_options();
    $options = array_filter($settings, "awards_options_filter");
    foreach ($options as $option) :
        if(isset($option['action'])){
            ...
            elseif( $option[ 'type' ] == 'colorpicker' ){   
                ...
                $rgb = awards_hex2rgb($colorpicker);
